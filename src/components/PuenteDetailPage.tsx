...
import { getPTStatusFromMariaDB, PTStatusData } from '../services/api';
...
const PuenteDetailPage = () => {
  ...
  // Función para obtener los datos del PT desde MariaDB
  const fetchPTData = async () => {
    try {
      const data = await getPTStatusFromMariaDB();
      setPTData(data);
      
      // Si la posición es diferente de 0, la guardamos como la última posición válida
      if (data && data.posicion !== 0) {
        setLastValidPosition(data.posicion);
      }
      
      console.log('Datos del PT obtenidos:', data, 'Última posición válida:', data?.posicion !== 0 ? data.posicion : lastValidPosition);
    } catch (error) {
      console.error('Error al obtener datos del PT:', error);
    }
  };
                    ...
                    <div className="bg-white rounded-lg shadow-md p-4 border border-gray-200">
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-medium">{ptData?.posicion === 0 ? lastValidPosition : ptData?.posicion || lastValidPosition}</span>
                        </div>
                        {ptData?.posicion === 0 && (
